...
**同样一件事,不必再调一次大模型** —— Omnilimb 直接确定性执行:结果一样,token 账单不一样。
...
- 💸 **零额外 token。** 执行路径从不调用模型——智能体只决策*一次*,Omnilimb
  确定性地把活干完再把 JSON 交回。更省钱、更快、可复现。
- 🛡️ **确定性 · 安全。** 重试、回滚、可选审计日志;Docker 沙箱默认关闭网络,
  路径穿越与 zip-slip 均有防护。
- 🧠 **会转换、会学习。** 任何技能转成原生 Hermes 技能——或用一句话学个新技能。

无锁定、不回传(搜索只与你选定的市场通信),**有没有 Node 都能跑**,且**支持多市场**
——详见下方 [选择市场](#选择市场) 与 [选择后端](#选择后端)。
...
一组智能体可直接调用的结构化 JSON 工具,按三件事归类:

**🧩 接入并运行社区技能** —— 按 slug / git 仓库 / 本地路径查找并安装(带校验),
确定性运行技能入口(重试 + 回滚,不跑第二层 AI),跨市场发现并对单个技能做
**体检**,以及管理已安装的一切(查看/编辑、冒烟测试、导入/导出)。**有没有 Node
都能跑**(双后端)。

**🛡️ 安全地执行** —— 隔离的 **Docker 沙箱**(默认断网、即时回滚)、用结构化动作
列表驱动 **Playwright** 浏览器、多语言运行时(python / node / bash / ruby / go),
以及可选的 JSONL **审计日志**。

**✨ 转换 & 学习 · 1.0 新** —— 把已安装的市场技能转换成原生 Hermes 技能(确定性
或 AI 策展),或从任意来源(路径、URL、粘贴文本)**学习**一个。还有确定性、不调
大模型的 **技能管家**,为你体检、推荐、诊断,并内置学习技能。
...
> **开放 1.0 —— 每个工具和仪表盘功能都免费,采用 MIT。没有付费层,无需解锁 —— 永久免费。**
